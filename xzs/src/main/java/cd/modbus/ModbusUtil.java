...
public class ModbusUtil {
    ...
    public static int readRegister(String ip, int port, int address,int slaveId)throws Exception {
        int data = 0;
        try {
            InetAddress addr = InetAddress.getByName(ip);  //地址

            TCPMasterConnection con = new TCPMasterConnection(addr);  //开启tcp

            con.setPort(port); //端口
            con.connect(); //连接
            ReadMultipleRegistersRequest req = new ReadMultipleRegistersRequest(address, 1);  //地址  读取数
            req.setUnitID(slaveId);
